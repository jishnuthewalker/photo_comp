  ...
  temporal(
    (set) => ({
      ...
      reorderPhotosMulti: (ids, toIndex) =>
        set((s) => {
          ...
          if (targetId === undefined || idSet.has(targetId)) {
            // Dropped out of range or onto a moving member — anchor to prefix length
            ...
            // Moving down → land after target; moving up → land before target
      ...
      loadProject: (project) => {
        ...
        // Clear undo history so the user can't undo back to the previous project
        setTimeout(() => useProjectStore.temporal.getState().clear(), 0);
      },

      // ── Selection actions (no touch — selection is not project data) ───────
      ...
      selectRange: (toIndex) =>
        set((s) => {
          ...
          const anchorIndex = s.selectionAnchorId
            ...
            : -1;
          if (anchorIndex === -1) {
            // Anchor gone or unset → treat as single select, set anchor
    ...
    {
      // Track undo for meaningful state changes only; selection lives outside `project`
      // so it is naturally excluded from temporal history.
